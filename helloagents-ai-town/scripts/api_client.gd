...
func send_chat(npc_name: String, message: String) -> void:
	...
	# Godot 的 get_datetime_dict_from_unix_time 返回 UTC 时间，需要手动加8小时转为北京时间
	var utc_dt = Time.get_datetime_dict_from_unix_time(int(now))
	var beijing_hour = (utc_dt.hour + 8) % 24
	print("[API] POST /chat -> ", data, " at %02d:%02d:%02d" % [beijing_hour, utc_dt.minute, utc_dt.second])
